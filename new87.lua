...
local sNames = {"textUpdateToggle", "textUpdateDelay", "serverHopToggle", "serverHopDelay", "colorBox", "goalBox", "webhookToggle", "webhookBox", "danceChoice", "thanksMessage", "signToggle", "customBoothText", "signUpdateToggle", "signText", "signColorBox", "autoThanks", "autoBeg", "begMessage", "begDelay", "fpsLimit", "render", "thanksDelay", "vcServer", "randomToggle", "webhookshop", "anonymous", "fontFace", "fontSize", "staffHop", "censorHop", "reex", "boothPosition", "altServers", "shortMessage"}
local sValues = {true, 30, true, 0, "0.196078, 0.803922, 0.196078", 5, false, "", "Disabled", {"Thank you", "Thanks!", "ty :)", "tysm!"}, false, "GOAL: $G", false, "your text here", "1, 1, 1", true, false, {"Please donate", "I'm so close to my goal!", "donate to me", "please"}, 300, 60, false, 3, false, true, false, false, "GothamBlack", 90, true, false, true, 3, false, false}
...
local function serverHop()
    if getgenv().settings.webhookshop then
        if getgenv().settings.shortMessage then
            webhook(tostring(Players.LocalPlayer.DisplayName..": hopped"))
        else
            webhook(tostring("["..Players.LocalPlayer.DisplayName.."] Attempting to server hop"))
        end
    end
    local gameId = "8737602449"
    if vcEnabled and getgenv().settings.altServers and game.PlaceId == 8737602449 then
        gameId = "8943844393"
    end
...
local function update(ok, force)
    ...
    elseif goal > 9999 then
        if tonumber(getgenv().settings.goalBox) < 100 then
            goal = string.format("%.1fk", (current + 100) / 10 ^ 3)
        else
    ...
    if getgenv().settings.signToggle and getgenv().settings.signUpdateToggle and getgenv().settings.signText and signPass and ok ~= "booth" then
        local currentSign = Players.LocalPlayer.Character.DonateSign.TextSign.SurfaceGui.TextLabel.Text
...
local webhookshop = webhookTab:Toggle("Short Messages", "Makes webhook messages shorter", getgenv().settings.shortMessage, function(t)
    if settingsLock then return end
    getgenv().settings.shortMessage = t
    saveSettings()
end)
...
serverHopTab:Toggle("Re-Execute", "Executes the script after server hop\nTurn this off if you use autoexec", getgenv().settings.reex, function(t)
    if t and not exd then
        queueonteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/zaczoc000/1/main/new87.lua?token=GHSAT0AAAAAAB3PMZK4MQ4VVUF3AN7B5DLOY337PSQ'))()")
        exd = true
    end
    if settingsLock then return end
...
otherTab:Button("Discord Server", "Copies Discord server link", function()
    local serverLink = "https://discord.gg/APDF6KQ56H"
    cboard(serverLink)
    Flux:Notification("Copied to clipboard", serverLink, "Okay")
end)
...
otherTab:Textbox("Import Settings", "Paste your settings here to import", false, "", function(t)
    if settingsLock then return end
    writefile(settingFile, t)
    task.wait(1)
    Flux:closeUi()
    serverHop()
end)

otherTab:Button("Export Settings", "Copies the current settings to your clipboard", function()
    local exports = httpservice:JSONEncode(getgenv().settings)
    cboard(exports)
    Flux:Notification("Copied to clipboard", exports, "Okay")
end)
...
Players.LocalPlayer.leaderstats.Raised.Changed:Connect(function(newRaised)
    if getgenv().settings.webhookToggle and getgenv().settings.webhookBox then
        task.spawn(function()
            ...
            for i = 0, 4 do
                if string.find(logs[#logs - i].message, Players.LocalPlayer.DisplayName) then
                    message = logs[#logs - i].message
                    amount = string.gsub(string.match(message, 'tipped (.*) Robux'), ",", "")
                    if getgenv().settings.shortMessage then
                        webhook(tostring(Players.LocalPlayer.DisplayName..": "..amount.." *[A/T: "..math.floor(tonumber(amount) * 0.6).."]* (T: "..newRaised..")"))
                    else
            ...
            if not sent then
                amount = newRaised - RaisedC
                if getgenv().settings.shortMessage then
                    webhook(tostring(Players.LocalPlayer.DisplayName..": "..amount.." *[A/T: "..math.floor(amount * 0.6).."]* (T: "..newRaised..")"))
                else
            ...
            if getgenv().settings.thanksDelay then
                local donoName = ""
                if getgenv().settings.thanksDelay > 1 then
                    task.wait(getgenv().settings.thanksDelay - 1)
                end
                if message then
                    donoName = string.match(message, '💰 (.*) tipped')
                end
                local tymessage = string.gsub (getgenv().settings.thanksMessage[math.random(#getgenv().settings.thanksMessage)], "$D", donoName)
                game.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(tymessage,"All")
            end
