...
function Flux:Window(text, bottom,mainclr,toclose)
	...
	function Flux:Notification(mainTitle,desc,buttontitle)
		...
		NotificationTitle.Text = mainTitle
	...
	function Tabs:Tab(text,ico)
		...
		function ContainerContent:Dropdown(text,list,font,callback)
			...
			DropItemHolder.Size = UDim2.new(0, 436, 0, 150)
			...
			function DropFunc:Clear(clearText)
				Title.Text = text
				ItemCount = ItemCount - 1
				if ItemCount == 1 then
					FrameSize = 78
				...
				if clearText then
					for i, v in next, DropItemHolder:GetChildren() do
						if v.Name == "Item" and v.Text == clearText then
							v:Destroy()
						end
					end
				else
		...
		function ContainerContent:Label(text, textsize)
		...
		function ContainerContent:Textbox(text,desc,disapper,defaultText,callback)
			if desc == "" then
